...
**관리자 게시 요청 (2026-09-22T16:58:44.969Z)**: 저장된 초안 5에서 공개 선택한 전체 작업 117건·대표작 9건을 반영했다. 비공개 초안과 미선택 업로드는 제외했다. GitHub 변경 반영 후 Vercel 배포 완료 확인은 별도다.


**09-23 사용자 운영 검증 완료**: 사용자가 운영 도메인에서 본인 Google 로그인·초안 저장·미리보기·게시·Vercel 후속 배포·다른 Google 계정 로그인 거부를 모두 직접 확인했다(「정상 작동해 다」). `docs/admin-management.md`의 미배포·권한 검증 진행 중 설명을 완료 상태로 갱신했다. 테스트 redirect는 09-23 사용자가 Supabase에서 제거하고 새로고침 후 목록에 없음을 확인했다. 에이전트는 대시보드 최종 목록을 열람하지 못했으며 운영 redirect·Site URL 유지 원칙은 그대로다.

**관리자 게시 요청 (2026-09-22T15:59:10.395Z)**: 저장된 초안 3에서 공개 선택한 전체 작업 117건·대표작 9건을 반영했다. 비공개 초안과 미선택 업로드는 제외했다. GitHub 변경 반영 후 Vercel 배포 완료 확인은 별도다.
...
실제 Google 인증 세션으로 **운영 Vercel API**의 `load`와 `prepare` HTTP 200 확인: 저장 초안 revision 1, 대표작 9개·아카이브 117개·삭제 0개. 이 검사는 읽기·게시 준비만 실행했으며 관리자 `publish`는 누르지 않았다. 이후 운영 도메인에서의 사용자 직접 Google 로그인, 실제 게시 클릭에 따른 후속 배포, 다른 실제 Google 계정 로그인 거부는 위 사용자 운영 검증으로 완료됐다. `http://127.0.0.1:4423/admin` 테스트 redirect는 09-23 사용자가 Supabase에서 제거했다. 아래 미배포·미연결 표기는 당시 기록이다.
