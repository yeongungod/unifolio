...
1인 사업자 「우니스튜디오」(영상 편집·사운드 믹싱, 대표 박영운)의 포트폴리오 사이트. 목적은 **거래처가 작업물과 연락처를 확인하고, A4 한 장 소개서를 받아 가는 것**이다. 공개 페이지는 정적이며, 09-22 사용자 요청으로 `/admin` 포트폴리오 관리 기능을 구현·연결 중이다. 블로그·문의 폼은 없다.
...
| 열려 있는 일 | 메모 |
|---|---|
| 대표작 9건 구성·재선정 | ✅ 09-22 사용자 지정 9개 순서 반영. 6번 레이븐어스는 해군·공군 기존 카드. 로컬 미배포 |
| 로그인 가능한 관리자 페이지 | ⏳ 로컬 구현·화면 검증. Supabase 프로젝트 생성·SQL 적용 성공, Google 로그인·권한 실검증·게시 연결 진행 중. `docs/admin-management.md` |
